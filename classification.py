...
支出 = dict(
            食品餐饮=dict(
                ...
                |生鲜|蔬菜|菜铺|水产|干货|副食|豆制品|饮食|云吞|食堂|冻品|猪扒|潮发潮汕牛肉店|冻肉|肠粉|小面|云吞|米皮|[猪牛羊]肉|[鸡鸭鹅]肉|[鱼虾蟹]|
                [海鲜贝]|[蔬菜水果]|[蛋奶乳]|[饮料酒水]|[餐饮店铺]|[餐饮服务]|[餐饮连锁店]|[餐饮品牌])''',

                零食="零食|小吃|炒货|桥头排骨|卫龙|翻天娃|辣翻天|贤哥|饼干|(可丽||华夫|夹心|桃酥|鲜花|老婆|[雪月])饼|[果肉][脯干]|蜜饯|(盐津|良品)铺子|仙贝|浪味仙|妙脆角|([薯A]|山["
                            ...
                            "干坚糖腰白]|碧根|开心|无花|罗汉|夏威夷)果|巴旦木|果([冻酱]|丹皮)|[凤鸡]爪|[鸭鹅]掌|食品店|[臭香]豆腐|广隆蛋挞王,",
            ...
            起居生活=dict(
                网话费="电信|联通|移动|宽带|流量|话费|通信专营|营业厅",
                日用品="顺客隆|昌大昌|沃尔玛|美宜佳|永旺|洗衣|宜家|IKEA|洗衣液|洗衣粉|厨房|垫子|蒸笼|日用品|曰用品|全家|百货|粮油|便利店|超市|便利购",
                房租还贷="公寓|房租|租金|房东|莫庆培|酒店",
                水电煤="([电水]|燃气)费|液化石油气|液化气((?!阀|管).)*",
            ...
            形象=dict(
                ...
                衣裤鞋帽箱包="[衣裤鞋袜帽穿戴衫袖裙]|([遮太]阳|[眼墨])镜|([男女工时西上下正]|表演|社交|职业|运动|休闲|情侣|诱惑)装|T恤|皮草|披风|马甲|吊带|背心|肚兜|亵衣|[胸乳]罩|[文裹]胸|(羽绒|居家|作训|[常制汉]|工作|水手)服|外套|夹克|牛仔|"
                             "潮[牌流男]|耐克|nike|阿迪|361[度°]|鸿星尔克|特步|贵人鸟|七匹狼|李宁|美特斯邦威|优衣库|以纯|真维斯|花花公子|PLAYBOY|SEPTWOLVES|啄木鸟|TUCANO|恒源祥|hengyuanxiang|卡帝乐鳄鱼|CARTELO|班尼路|Baleno|嘉晔|JIAYE|罗蒙|"
                             "ROMON|皮尔卡丹|pierre ?cardin|富贵鸟|FUGUINIAO|布先生|森马|Semir|A21|拇指白小T|稻草人|迪卡侬",
            ),

            健康医疗=dict(
                医院="医([院馆生护用药]|.?室)|药([店房号业厂]|匣子)|诊所|卫生(服务(中心|站)|[院所室])|老百姓|海王星辰|(同仁|九芝)堂|甘之草科技|三七|黄芪|枸杞|菊花|甘草|红花|当归|石斛|凉茶|([白红沙玄党苦人]|西洋|花旗|太子)参|海(马|狗肾)|医院",
            ),
...
def classify_transaction(in_or_out, search_text):
    """
    纯分类函数，只负责根据文本匹配分类规则
    
    Args:
        in_or_out: "支出" 或 "收入"
        search_text: 要匹配的文本（已由调用方组合好）
    
    Returns:
        tuple: (一级分类, 二级分类) 或 ("未分类", "未分类")
    """
    if in_or_out == "支出":
        for category in 支出:
            for sub_category in 支出[category]:
                pattern = 支出[category][sub_category]
...
if __name__ == '__main__':
    in_or_out = "支出"
    store = "汽车"
    transaction_type = "腾达汽修"
    cc = classification(in_or_out, store, transaction_type)
    print(cc)
